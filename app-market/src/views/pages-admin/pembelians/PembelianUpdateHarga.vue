    ...
    <transition>
        <div v-if="dialog" class="fixed inset-0 bg-basic-800 bg-opacity-70 z-10 flex items-center justify-center">
            <div class="md:w-1/4 shadow bg-basic-50 h-max-[450px]">
                <div class="py-1 px-5 font-semibold border-b">Update Harga</div>
                <div class="py-3 px-5">
                    <form @submit.prevent="simpan">
                        <div class="grid grid-cols-1">
                            <input type="number" ref="refHarga" step="any" v-model="form.harga"
                                   class="py-1 px-2 outline-none focus:border-basic-500 border border-basic-200 w-full"
                                   placeholder="NB: 10%">
                            <ValidateError :errors="errors" name="harga"></ValidateError>
                        </div>
...
<script setup>
    ...
    async function simpan() {
        let {data} = await bsoft.axios.post('/pembelian/detail/harga/' + form.id, form);
        if (data.code === 200) {
            sukses(data);
        } else {
