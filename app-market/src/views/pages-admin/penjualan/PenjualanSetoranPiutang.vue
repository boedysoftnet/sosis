        ...
        <div v-if="dialog" class="fixed inset-0 bg-basic-800 bg-opacity-70 z-10 flex items-center justify-center">
            <div class="md:w-1/2 shadow bg-basic-50 h-max-[450px]">
                ...
                <div class="py-3 px-5">
                    <div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Nama Customer</label>
                            <div class="flex-1 text-left">{{dataRef.penjualan.customer.nama}}</div>
                        </div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Alamat</label>
                            <div class="flex-1 text-left">{{dataRef.penjualan.customer.alamat}}</div>
                        </div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Telpon</label>
                            <div class="flex-1 text-left">{{dataRef.penjualan.customer.telpon}}</div>
                        </div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Total</label>
                            <div class="flex-1 text-left">
                                {{bsoft.NumberFormat(dataRef.penjualan.total_setelah_ppn,0)}}
                            </div>
                        </div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Piutang</label>
                            <div class="flex-1 text-left">{{bsoft.NumberFormat(dataRef.penjualan.piutang,0)}}</div>
                        </div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Sisa</label>
                            <div class="flex-1 text-left">{{bsoft.NumberFormat(dataRef.penjualan.sisa_piutang,0)}}</div>
                        </div>
                        <div class="flex">
                            <label for="" class="w-52 text-left">Terbayar</label>
                            <div class="flex-1 text-left">{{bsoft.NumberFormat(dataRef.penjualan.terbayar,0)}}</div>
                        </div>
                    </div>
...
<script setup>
    ...
    function showDialog(id = null) {
        ...
        bsoft.axios.get(`/penjualan/${id}`).then(res => {
            if (res.data.code === 200) dataRef.penjualan = res.data.data;
        })
    ...
    function dismisDialog() {
        dialog.value = false;
        emit('onRefresh',dataRef.penjualan.id);
    }

    function simpanSetoran() {
        if(dataRef.penjualan.sisa_hutang<form.jumlah){
            Swal.fire({
                icon:'warning',
                title:'Peringatan...!',
            ...
            form.penjualan_id = dataRef.penjualan.id;
            bsoft.axios.post('/penjualan/piutang/setoran', form).then(res => {
