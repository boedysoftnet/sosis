        ...
        <div v-if="dialog" class="fixed inset-0 bg-basic-800 bg-opacity-50 flex items-center justify-center z-10">
            <div class="w-full md:w-1/2 bg-basic-50">
                <div class="py-2 px-2 border-b border-basic-300 font-semibold uppercase flex justify-between items-center">
                    <div><a href="" class="dx-icon-back" @click.prevent="dismiss()"></a> Pilih Supplier</div>
                ...
                <div class="py-2 px-2">
                    <div class="mb-2">
                        <input type="search"
                               ...
                               v-model="cari" placeholder="Cari Supplier..!">
                    </div>
                    <dx-data-grid :data-source="dataSource.data"
                        ...
                        <dx-column data-field="ppn"/>
                    </dx-data-grid>
                    <div class="mt-2 border-t border-basic-300 pt-2 flex justify-between">
                        <button type="button" class="py-1 px-2 bg-basic-500 text-basic-50" @click.prevent="refSupplierRegister.showDialog()"><span class="dx-icon-plus"></span>Buat Supplier</button>
                        <button class="py-1 px-2 bg-blue-500 text-basic-50" @click.prevent="dismiss"><span class="dx-icon-back"></span>Tutup</button>
                    </div>
                    <supplier-register ref="refSupplierRegister" @onRefresh="refresh" class="text-left"></supplier-register>
                </div>
            </div>
        </div>
    </transition>
...
<script setup>
    ...
    import supplierRegister from '../../views/pages-admin/suppliers/SupplierRegister.vue';
    ...
    function refresh() {
        dataSource.data = new CustomStore({
            key: 'id',
            load: async () => {
                let {data} = await bsoft.axios.get('/supplier',{params:{cari:cari.value,limit:50}});
                return data.data;
            }
        })
    }
    ...
    function pilih({selectedRowsData}) {
        emit('setSupplier',selectedRowsData[0].id);
        dismiss();
    }
